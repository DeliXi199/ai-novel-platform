...
def current_model() -> str:
    provider = provider_name()
    if provider == "openai":
        return normalize_text(settings.openai_model) or "gpt-5.4"
    if provider == "deepseek":
        return normalize_text(settings.deepseek_model) or "deepseek-chat"
    if provider == "groq":
        return normalize_text(settings.groq_model) or "openai/gpt-oss-20b"
...
def get_llm_runtime_config() -> dict[str, Any]:
    ...
    return {
        "provider": provider,
        "model": current_model() if provider in {"openai", "deepseek", "groq"} else None,
        "base_url": current_base_url(),
        "api_key_present": bool(current_api_key()),
        "api_key_masked": mask_secret_tail(current_api_key()),
...
def ping_generation_provider() -> dict[str, Any]:
    ...
    return {**get_llm_runtime_config(), "ping_text": text[:32]}
...
def get_client() -> Any:
    ...
    require_generation_provider(stage="provider_check")
    provider = provider_name()
    signature = (provider, current_api_key(), current_base_url())
...
def call_text_response(*, stage: str, system_prompt: str, user_prompt: str, max_output_tokens: int | None = None) -> str:
    require_generation_provider(stage=stage)
    client = get_client()
    provider = provider_name()
    output_tokens = max_output_tokens or current_max_output_tokens()
    request_kwargs: dict[str, Any] = {"model": current_model()}
    if provider == "deepseek":
    ...
    try:
        response = client.chat.completions.create(**request_kwargs) if provider == "deepseek" else client.responses.create(**request_kwargs)
    except APITimeoutError as exc:
        duration_ms = int((time.perf_counter() - started) * 1000)
        details = {"trace_id": _trace_id_var.get(), "duration_ms": duration_ms, **extract_api_error_details(exc)}
    ...
    duration_ms = int((time.perf_counter() - started) * 1000)
    text = chat_completion_to_text(response) if provider == "deepseek" else response_to_text(response)
...
def call_json_response(*, stage: str, system_prompt: str, user_prompt: str, max_output_tokens: int | None = None) -> dict[str, Any]:
