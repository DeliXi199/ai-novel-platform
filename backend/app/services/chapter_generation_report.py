    ...
    *,
    ...
    if (avg_cards > 0 and latest_cards >= max(int(round(avg_cards * 1.5)), int(avg_cards) + 2)) or (
        ...
        _push_alert(
            ...
            severity="low",
            title="筛选规模开始膨胀",
            message=f"最新一章用卡 {latest_cards}、prompt 策略 {latest_prompts}，已经高于最近窗口均值，可能会继续推高上下文和调用成本。",
    ...
    return {
        ...
        "preparation": {
            ...
            "selected_outputs": {
                ...
                "selected_scene_templates": _safe_int(selected_outputs.get("selected_scene_templates")),
...
def compact_generation_pipeline_report(raw: dict[str, Any] | None) -> dict[str, Any]:
    ...
    compact = {
        ...
        "preparation_summary": [_truncate_text(item, 120) for item in (preparation.get("readable_lines") or [])[:3] if _text(item)],
        "selected_outputs": {
            "selected_cards": _safe_int(selected_outputs.get("selected_cards")),
            "selected_scene_templates": _safe_int(selected_outputs.get("selected_scene_templates")),
            "selected_prompt_strategies": _safe_int(selected_outputs.get("selected_prompt_strategies")),
